...
Rcpp::List selectCrossPlan(arma::uword cycleNumber,
  ...
  arma::uword nSnp = M.n_cols;
  arma::uword nInd = M.n_rows;
  arma::uword potCross = nInd * (nInd - 1) / 2;

  arma::umat crossPlan(2, nPop);
  arma::umat outCrossPlan(nCross, 3);
  arma::umat Progeny(nCross, nPop), Parents(nCross, nSel);
  arma::uvec Best(nCross);

  arma::vec uProgeny(nPop), uParents(nSel);
  arma::vec simProgeny(nPop), simParents(nSel);
  arma::vec angleProgeny(nPop), angleParents(nSel);
  arma::vec lenProgeny(nPop), lenParents(nSel);
  arma::vec valProgeny(nPop), valParents(nSel);
  arma::uvec rankProgeny(nPop);

  double uBest, simBest, valBest, angleBest, lenBest;
  double uMax, uMin, simMax, simMin;
  arma::uword currentRun;

  arma::uvec rowPos(potCross), colPos(potCross);
  arma::uword tmpI = 0;
  ...
  if(a.n_elem != nSnp || d.n_elem != nSnp){
    Rcpp::stop("Length of additive/dominance effects does not match number of SNPs.");
  }
  ...
  arma::mat G = (Mc * Mc.t()) / Mc.n_cols + offset;
  ...
  arma::mat Mt = Mc.t() + ploidy / 2.0;
  arma::vec u(potCross, arma::fill::zeros);

  for(arma::uword i = 0; i < potCross; ++i){
    for(arma::uword m = 0; m < nSnp; m++){

      double gi_d = Mt(m, rowPos(i));
      double gj_d = Mt(m, colPos(i));

      if(gi_d < 0 || gi_d > ploidy || gj_d < 0 || gj_d > ploidy){
        Rcpp::stop("Marker dosage outside expected 0:ploidy range.");
      }

      arma::uword gi = static_cast<arma::uword>(std::round(gi_d));
      arma::uword gj = static_cast<arma::uword>(std::round(gj_d));

      if(std::abs(gi_d - double(gi)) > 1e-8 ||
         std::abs(gj_d - double(gj)) > 1e-8){
        Rcpp::stop("Marker dosage is not integer-coded.");
      }
  ...
  arma::vec x = calcContr(uBestIndex.unsafe_col(0), nInd, rowPos, colPos);
  uMax = mean(u(uBestIndex));
  simMax = as_scalar(x.t() * G * x);
  ...
  for(arma::uword i = 0; i < nPop; i++){
    Progeny.col(i) = sampleInt(nCross, potCross);
    arma::vec x = calcContr(Progeny.unsafe_col(i), nInd, rowPos, colPos);
    simProgeny(i) = as_scalar(x.t() * G * x);
  }
  ...
  for(arma::uword gen = 0; gen < maxGen; gen++){
    ...
    for(arma::uword i = 0; i < nPop; i++){
      ...
      if(tmp.n_elem != nCross || arma::any(tmp >= potCross)){
        Rcpp::stop("Invalid cross index generated during mate().");
      }
      ...
      if(arma::any(Progeny.col(i) >= potCross)){
        Rcpp::stop("Invalid cross index generated during mutate().");
      }
  ...
  for(arma::uword i = 0; i < nPop; i++){
    ...
    valProgeny(i) =
      lenProgeny(i) - anglePenalty * fabs(angleProgeny(i) - targetAngle);
  }
